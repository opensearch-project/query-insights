...
public class LocalIndexReaderTests extends OpenSearchTestCase {
    ...
    public void testReadRecords() throws InterruptedException {
        ...
        assertEquals(timestamp, records1.get(0).getTimestamp());
        assertEquals(10000, records1.get(0).getMeasurement(MetricType.CPU));
        assertEquals(20000, records1.get(0).getMeasurement(MetricType.MEMORY));
        assertEquals(3, records1.get(0).getMeasurement(MetricType.LATENCY));
        Object[] indices = (Object[]) records1.get(0).getAttributes().get(Attribute.INDICES);
        assertNotNull(indices);
        assertEquals(1, indices.length);
        assertEquals("my-index-0", indices[0]);
        // Verify verbose-only fields are present for verbose=true
        assertTrue(records1.get(0).getAttributes().containsKey(Attribute.SOURCE));
        assertNotNull(records1.get(0).getAttributes().get(Attribute.SOURCE));
        assertTrue(records1.get(0).getAttributes().containsKey(Attribute.TASK_RESOURCE_USAGES));
        assertNotNull(records1.get(0).getAttributes().get(Attribute.TASK_RESOURCE_USAGES));
        assertTrue(records1.get(0).getAttributes().containsKey(Attribute.PHASE_LATENCY_MAP));
        assertNotNull(records1.get(0).getAttributes().get(Attribute.PHASE_LATENCY_MAP));
        ...
        SearchQueryRecord record2 = records2.get(0);
