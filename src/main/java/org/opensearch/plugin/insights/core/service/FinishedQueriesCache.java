...
public class FinishedQueriesCache {
    ...
    public void addFinishedQuery(SearchQueryRecord record) {
        ...
        if (currentTime - lastAccessTime > TRACKING_INACTIVITY_MS) {
            return;
        }
        synchronized (finishedQueries) {
            // Clean up expired queries before adding new one
            while (!finishedQueries.isEmpty() && (currentTime - finishedQueries.peekFirst().finishTime) > FINISHED_QUERY_RETENTION_MS) {
