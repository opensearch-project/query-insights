...
public class LiveQueryRecord {
    ...
    /**
     * Convert to SearchQueryRecord for API compatibility
     */
    public SearchQueryRecord toSearchQueryRecord() {
        Map<MetricType, Measurement> measurements = new HashMap<>();
        measurements.put(MetricType.LATENCY, new Measurement(latencyNanos));
        measurements.put(MetricType.CPU, new Measurement(cpuNanos));
        measurements.put(MetricType.MEMORY, new Measurement(memoryBytes));

        Map<Attribute, Object> attributes = new HashMap<>();
        attributes.put(Attribute.NODE_ID, nodeId);
        attributes.put(Attribute.START_TIME, startTime);
        attributes.put(Attribute.END_TIME, endTime > 0 ? endTime : null);
        attributes.put(Attribute.IS_CANCELLED, cancelled);
        ...
        return new SearchQueryRecord(startTime, measurements, attributes, taskId);
    }
}
